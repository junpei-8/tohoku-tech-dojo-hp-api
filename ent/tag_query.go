...
// QueryNewsList chains the current query on the "news_list" edge.
func (tq *TagQuery) QueryNewsList() *NewsQuery {
	query := &NewsQuery{config: tq.config}
...
func (tq *TagQuery) QueryNewsTagging() *NewsTaggingQuery {
	query := &NewsTaggingQuery{config: tq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := tq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (tq *TagQuery) Clone() *TagQuery {
	...
	return &TagQuery{
		...
		withNewsList:    tq.withNewsList.Clone(),
		withNewsTagging: tq.withNewsTagging.Clone(),
...
// WithNewsList tells the query-builder to eager-load the nodes that are connected to
// the "news_list" edge. The optional arguments are used to configure the query builder of the edge.
func (tq *TagQuery) WithNewsList(opts ...func(*NewsQuery)) *TagQuery {
	query := &NewsQuery{config: tq.config}
	for _, opt := range opts {
		opt(query)
	}
	tq.withNewsList = query
	return tq
}
...
func (tq *TagQuery) WithNewsTagging(opts ...func(*NewsTaggingQuery)) *TagQuery {
	query := &NewsTaggingQuery{config: tq.config}
	for _, opt := range opts {
		opt(query)
	}
	tq.withNewsTagging = query
	return tq
}
...
func (tq *TagQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Tag, error) {
	...
	if query := tq.withNewsList; query != nil {
		if err := tq.loadNewsList(ctx, query, nodes,
			func(n *Tag) { n.Edges.NewsList = []*News{} },
			func(n *Tag, e *News) { n.Edges.NewsList = append(n.Edges.NewsList, e) }); err != nil {
			return nil, err
		}
	}
	if query := tq.withNewsTagging; query != nil {
		if err := tq.loadNewsTagging(ctx, query, nodes,
			func(n *Tag) { n.Edges.NewsTagging = []*NewsTagging{} },
			func(n *Tag, e *NewsTagging) { n.Edges.NewsTagging = append(n.Edges.NewsTagging, e) }); err != nil {
			return nil, err
		}
	}
	for name, query := range tq.withNamedNewsList {
		if err := tq.loadNewsList(ctx, query, nodes,
			func(n *Tag) { n.appendNamedNewsList(name) },
			func(n *Tag, e *News) { n.appendNamedNewsList(name, e) }); err != nil {
			return nil, err
		}
	}
	for name, query := range tq.withNamedNewsTagging {
		if err := tq.loadNewsTagging(ctx, query, nodes,
			func(n *Tag) { n.appendNamedNewsTagging(name) },
			func(n *Tag, e *NewsTagging) { n.appendNamedNewsTagging(name, e) }); err != nil {
			return nil, err
		}
	}
...
func (tq *TagQuery) loadNewsList(ctx context.Context, query *NewsQuery, nodes []*Tag, init func(*Tag), assign func(*Tag, *News)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(tag.NewsListTable)
		s.Join(joinT).On(s.C(news.FieldID), joinT.C(tag.NewsListPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(tag.NewsListPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(tag.NewsListPrimaryKey[1]))
	...
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "news_list" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
...
func (tq *TagQuery) loadNewsTagging(ctx context.Context, query *NewsTaggingQuery, nodes []*Tag, init func(*Tag), assign func(*Tag, *NewsTagging)) error {
	...
	query.Where(predicate.NewsTagging(func(s *sql.Selector) {
		s.Where(sql.InValues(tag.NewsTaggingColumn, fks...))
	}))
	...
	for _, n := range neighbors {
		...
		node, ok := nodeids[fk]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "tag_id" returned %v for node %v`, fk, n)
		}
		assign(node, n)
	}
	return nil
...
// WithNamedNewsList tells the query-builder to eager-load the nodes that are connected to the "news_list"
// edge with the given name. The optional arguments are used to configure the query builder of the edge.
func (tq *TagQuery) WithNamedNewsList(name string, opts ...func(*NewsQuery)) *TagQuery {
	query := &NewsQuery{config: tq.config}
	for _, opt := range opts {
		opt(query)
	}
	if tq.withNamedNewsList == nil {
		tq.withNamedNewsList = make(map[string]*NewsQuery)
	}
	tq.withNamedNewsList[name] = query
	return tq
}

// WithNamedNewsTagging tells the query-builder to eager-load the nodes that are connected to the "news_tagging"
// edge with the given name. The optional arguments are used to configure the query builder of the edge.
func (tq *TagQuery) WithNamedNewsTagging(name string, opts ...func(*NewsTaggingQuery)) *TagQuery {
	query := &NewsTaggingQuery{config: tq.config}
	for _, opt := range opts {
		opt(query)
	}
	if tq.withNamedNewsTagging == nil {
		tq.withNamedNewsTagging = make(map[string]*NewsTaggingQuery)
	}
	tq.withNamedNewsTagging[name] = query
	return tq
}
