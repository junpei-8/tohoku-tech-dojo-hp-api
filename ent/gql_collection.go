	...
	for _, field := range graphql.CollectFields(op, field.Selections, satisfies) {
		...
		case "tags":
			var (
				alias = field.Alias
				path  = append(path, alias)
				query = &TagQuery{config: n.config}
			)
			if err := query.collectField(ctx, op, field, path, satisfies...); err != nil {
				return err
			}
			n.WithNamedTags(alias, func(wq *TagQuery) {
				*wq = *query
			})
		}
	}
...
func newNewsPaginateArgs(rv map[string]interface{}) *newsPaginateArgs {
	...
	if v, ok := rv[whereField].(*NewsWhereInput); ok {
		args.opts = append(args.opts, WithNewsFilter(v.Filter))
	}
	return args
}
	...
	for _, field := range graphql.CollectFields(op, field.Selections, satisfies) {
		...
		case "newsTags":
			var (
				alias = field.Alias
				path  = append(path, alias)
				query = &NewsQuery{config: t.config}
			)
			if err := query.collectField(ctx, op, field, path, satisfies...); err != nil {
				return err
			}
			t.WithNamedNewsList(alias, func(wq *NewsQuery) {
				*wq = *query
			})
		}
	}
...
func (u *UserQuery) collectField(ctx context.Context, op *graphql.OperationContext, field graphql.CollectedField, path []string, satisfies ...string) error {
	...
	for _, field := range graphql.CollectFields(op, field.Selections, satisfies) {
		switch field.Name {
		case "createdNewsList", "created_news_list":
			var (
				alias = field.Alias
				path  = append(path, alias)
			...
			u.WithNamedCreatedNewsList(alias, func(wq *NewsQuery) {
				*wq = *query
			})
		}
	}
