...
// The NewsTaggingFunc type is an adapter to allow the use of ordinary
// function as NewsTagging mutator.
type NewsTaggingFunc func(context.Context, *ent.NewsTaggingMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f NewsTaggingFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	mv, ok := m.(*ent.NewsTaggingMutation)
	if !ok {
		return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.NewsTaggingMutation", m)
	}
	return f(ctx, mv)
}
